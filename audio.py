...
bot = commands.Bot(command_prefix=get_prefix,description='A music bot for discord Python Music')
                ...
                if len(bot.voice_client_in(before.server).channel.voice_members) <= 1: #there is only bot in voice channel
                    if player_status[before.server.id]==True:
                        ...
                        if len(bot.voice_client_in(before.server).channel.voice_members) <= 1:
                            ...
                            await bot.send_message(discord.Object(id=rq_channel[before.server.id]),"**Python Music left because there was no one inside `{}`**".format(before.voice.voice_channel))
